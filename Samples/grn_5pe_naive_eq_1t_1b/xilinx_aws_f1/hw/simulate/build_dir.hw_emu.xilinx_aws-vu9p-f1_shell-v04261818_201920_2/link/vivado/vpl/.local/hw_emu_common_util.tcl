  ...
  proc compile_scripts_step { config_info } {
    ...
    set cur_pwd [pwd]
     ...
     if {[string compare -nocase $origSimulator "xcelium"] == 0
         || [string compare -nocase $origSimulator "vcs"] == 0} {
        set isMsgCheck 1      
   ...
   if { [dict exists $config_info emu_proj_name] } {
    ...
    if {$isNewXsa && $is_new_flow} {
      ...
      foreach bdDes $bdDesigns {
        # puts "BD-design $bdDes"
        set bdName [file tail $bdDes]
        # puts "bdName : $bdName"
        # Do not tamper with scoped-BD or xlnoc BD
        # This should be data-driven (IS_TOOL_MANAGED property?)
        # TODO: revisit in HEAD
        if { [ string equal -nocase $bdName "xlnoc.bd"] } {
          continue
        }
        # skip scoped-BDs
        set parentCompFile [get_property -quiet PARENT_COMPOSITE_FILE $bdDes]
        if { $parentCompFile != "" } {
          continue
        }
        # Only consider noc-scoped BDs for upgrade
        lappend l_bds $bdDes
        # xlnoc.bd doesnt have PARENT_COMPOSITE_FILE property, but GENERATE_SYNTH_CHECKPOINT property is readonly for it. So dont change it
        # hack to check if "GENERATE_SYNTH_CHECKPOINT" property is read only. if it is readonly property, readonly tag will be true. Then we should not set generate_synth_checkpoint manually.
        set val2 [report_property $bdDes -regexp .*GENERATE_SYNTH_CHECKPOINT.* -return_string]
        if {[string first "true" $val2] >= 0} {
            puts "---NOT setting synth checkpoint false for bdDes $bdDes"
        } else {
          puts "---setting synth checkpoint false for bdDes $bdDes"
         set_property generate_synth_checkpoint false [get_files $bdDes]
        }
      }
      ...
      foreach bdDes $l_bds {
        open_bd_design -auto_upgrade [get_files $bdDes]
      }
    ...
    if {[string first "ignore_assertions" $current_value] == -1 } {
      ...
      #if {[string compare -nocase $origSimulator "xcelium"] == 0} {
      #  set isMsgCheck 1      
      #  compile_and_elaborate $isMsgCheck
      #} else {
      #  compile_and_elaborate ""
      #}
